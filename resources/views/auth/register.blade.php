...
<div class="layadmin-user-login-main">
    ...
    <form method="POST" action="{{ route('register') }}" class="layadmin-user-login-box layadmin-user-login-body layui-form">
        @csrf
        <div class="layui-form-item">
            <label class="layadmin-user-login-icon layui-icon layui-icon-username" for="LAY-user-name"></label>
            <input type="text" name="name" id="LAY-user-name" lay-verify="required" placeholder="姓名" class="layui-input{{ $errors->has('name') ? ' layui-form-danger' : '' }}">
                @if ($errors->has('name'))
        ...
        <div class="layui-form-item">
                <button class="layui-btn layui-btn-fluid" lay-submit="" lay-filter="LAY-user-login-submit">{{ __('Register') }}</button>
        </div>
        <div><span><a href="{{ route('login') }}">已有帐号，点击登录。</a></span></div>
    </form>
</div>
@endsection
